...
export const GitWorktreePlugin: Plugin = async (ctx) => {
  ...
  const findWorktreeByName = async (name: string): Promise<string | undefined> => {
    const result = await $`git worktree list --porcelain`.quiet().nothrow().cwd(repoRoot);
  ...
  for (let attempt = 0; attempt < MAX_NAME_RETRIES; attempt++) {
    ...
    if (result.exitCode === 0) {
      ...
      if (startCommand) {
        ...
        const cmdResult = await $`bash -lc ${startCommand}`.quiet().nothrow().cwd(worktreeDir);
...
async function removeWorktree(
  ...
  const result = await $`git worktree remove --force ${directory}`.quiet().nothrow().cwd(repoRoot);
  ...
  const branchResult = await $`git branch -D ${branch}`.quiet().nothrow().cwd(repoRoot);
...
async function mergeWorktree(
  ...
  const checkoutResult = await $`git checkout ${targetBranch}`.quiet().nothrow().cwd(repoRoot);
  ...
  if (mergeResult.exitCode !== 0) {
    if (mergeStderr.includes("CONFLICT") || mergeStdout.includes("CONFLICT")) {
      ...
      const statusResult = await $`git status --porcelain`.quiet().nothrow().cwd(repoRoot);
  ...
  if (strategy === "manual") {
    const commitResult = await $`git commit -m ${message}`.quiet().nothrow().cwd(repoRoot);

    if (commitResult.exitCode !== 0 && !getStderr(commitResult).includes("nothing to commit")) {
      return {
        success: false,
